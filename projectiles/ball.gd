...
var damage: float
var animation_name: String = "fireball"
...
var time_to_hit: float = 1 # In seconds
...
func _process(delta: float) -> void:
	...
	if is_instance_valid(target):
		...
		if elapsed_time >= time_to_hit:
			...
			# Do we need to create a leech effect?
			if animation_name == "leech":
				# We do
				var l: Node2D = load("res://projectiles/LeechEffect.tscn").instantiate()
				l.heal_amount = (damage/2)
	...
	## To make it arc, we'll mess with the y axis
		...
		#vel.y -= (elapsed_time*50)
	#else:
		#vel.y -= ((time_to_hit/2) - (elapsed_time - (time_to_hit/2))) * 50
	global_position = start_position + vel
	# Angle our ourselves based on our vel
	$AnimatedSprite2D.rotation = (global_position-start_position).angle()
